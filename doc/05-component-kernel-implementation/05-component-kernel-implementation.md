...
***ANSWER: Sequence<String> will be used to implement Clipboard.\
   Convention: The most recently copied entry will be at postion 0, and the entries
   will become older as the index increases. There will be no null entries allowed
   and the length will correspond to the number of elements currently in the clipboard.\
   Correspondence: Each element in the sequence will represent a clipboard entry and
   the order of the entries corresponds to the recency of them. This will allow
   the most recent item to be retrieved much easier, and will allow for modifying
   older elements as well.\
   Justification: Sequence<String> is a good choice because it is build to easily
   maintain order which aligns with the clipboard operations, and the ease of accessing
   indicies also helps with many of the methods.***
